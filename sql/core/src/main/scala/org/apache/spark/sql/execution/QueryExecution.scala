...
import org.apache.spark.sql.extra.ResolveLogicalPlan
...
class QueryExecution(val sparkSession: SparkSession, val logical: LogicalPlan)
  ...
  lazy val sparkPlan: SparkPlan = {
    ...
    val debug = planner.plan(ReturnAnswer(optimizedPlan))
    var num = 1
    while (debug.hasNext) {
      val logicalPlan = debug.next()
      log.error("spark_plan_" + num + ":" + logicalPlan.toString)
      num = num + 1
    }
    log.error("optimized_plan:" + optimizedPlan.toString)
    val tuple = resolvePlan(optimizedPlan, "default")
    log.error("tuple:" + tuple.toString)
